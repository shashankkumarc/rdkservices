...
#define ARC_DETECTION_CHECK_TIME_IN_MILLISECONDS 3000
...
namespace WPEFramework {
    ...
    namespace Plugin {
        ...
        {   //sample servicemanager response: {"success":true,"supportedAudioPorts":["HDMI0"]}
            ...
            if ((getServiceState(m_service, HDMICECSINK_CALLSIGN, state) == Core::ERROR_NONE) && (state == PluginHost::IShell::state::ACTIVATED)) {
				...
				/*
				m_nextRequest = SEND_AUDIO_DEVICE_POWER_ON;//Should we send power request irrespective of System audio mode status
                        	std::lock_guard<std::mutex> lock(m_arcRoutingStateMutex);
                        	m_cecArcRoutingThreadRun = true;
                        	arcRoutingCV.notify_one();
				*/
				sendAudioDevicePowerOnMsg();
        ...
        {   //sample servicemanager response:
            ...
            try
            {
                ...
                if (!audioPort.empty())
                {
				    ...
				    if((types & dsAUDIOARCSUPPORT_ARC) && (m_hdmiInAudioDeviceConnected == true)) {
					if (mode == device::AudioStereoMode::kPassThru) {
					    if (m_AudioDeviceSADState  == AUDIO_DEVICE_SAD_CLEARED || \
							    m_AudioDeviceSADState  == AUDIO_DEVICE_SAD_UNKNOWN) {
						LOGINFO("%s: sending SAD request\n", __FUNCTION__);
                                                if (!DisplaySettings::_instance->sendShortAudioDescriptorRequestMsg()) {
                                                    success = false;
        ...
        {   //TODO: Handle other audio ports. Currently only supports HDMI ARC/eARC
            ...
            try
            {
                ...
                if(audioPort.compare("HDMI_ARC0")) {
                    ...
                    device::AudioOutputPort aPort = device::Host::getInstance().getAudioOutputPort(audioPort);
		    if (m_hdmiInAudioDeviceType == dsAUDIOARCSUPPORT_ARC && m_hdmiInAudioDeviceConnected == true && pEnable == true) {
			    if (m_AudioDeviceSADState == AUDIO_DEVICE_SAD_RECEIVED) {
			    ...
			    } else if (m_AudioDeviceSADState == AUDIO_DEVICE_SAD_REQUESTED && !(m_SADDetectionTimer.isActive())) {
				    ...
				    LOGINFO("%s: Not handling audio routing since m_hdmiInAudioDeviceConnected = %d \n",\
                                                __FUNCTION__, m_hdmiInAudioDeviceConnected);
				}
                            }
                            else{
				...
				if (m_hdmiInAudioDeviceConnected == false) {
			       ...
			       if( m_hdmiInAudioDeviceConnected == true) {
			           if(m_arcAudioEnabled == false ) {
				       if (m_AudioDeviceSADState == AUDIO_DEVICE_SAD_UPDATED) {
	...
	void  DisplaySettings::checkSADUpdate() {
		...
		if (m_AudioDeviceSADState == AUDIO_DEVICE_SAD_RECEIVED) {
		    ...
		    LOGINFO("%s: After SAD update enable Arc m_arcAudioEnabled = %d, m_hdmiInAudioDeviceConnected = %d\n", __FUNCTION__,m_arcAudioEnabled, m_hdmiInAudioDeviceConnected);
		    if( m_arcAudioEnabled == false && m_hdmiInAudioDeviceConnected == true) {
                        LOGINFO("%s: Enable ARC... after SAD update \n",__FUNCTION__);
                        aPort.enableARC(dsAUDIOARCSUPPORT_ARC, true);
                        m_arcAudioEnabled = true;
                    }
		} else {
		        LOGINFO("%s: SAD update still not received enable Arc m_arcAudioEnabled = %d, m_hdmiInAudioDeviceConnected = %d\n", __FUNCTION__,m_arcAudioEnabled, m_hdmiInAudioDeviceConnected);
			//Still SAD is not received, route audio with out SAD update.
		        if( m_arcAudioEnabled == false && m_hdmiInAudioDeviceConnected == true) {
                        LOGINFO("%s:Not recieved SAD update after 3sec timeout, Enable ARC... with default SAD\n",__FUNCTION__);
                        aPort.enableARC(dsAUDIOARCSUPPORT_ARC, true);
                        m_arcAudioEnabled = true;
                    }
		}
	...
	/* Message wrapper functions to push the message to queue  */
	bool DisplaySettings::sendShortAudioDescriptorRequestMsg ()
	{
	    SendMsgInfo msgInfo;
			
        	msgInfo.msg = REQUEST_SHORT_AUDIO_DESCRIPTOR;
        	msgInfo.param = -1;
		std::unique_lock<std::mutex> lock(DisplaySettings::_instance->m_sendMsgMutex);
	...
	void DisplaySettings::sendAudioDevicePowerStatusRequestMsg ()
	{
		SendMsgInfo msgInfo;
			
                msgInfo.msg = REQUEST_AUDIO_DEVICE_POWER_STATUS;
                msgInfo.param = -1;
		std::unique_lock<std::mutex> lock(DisplaySettings::_instance->m_sendMsgMutex);
                DisplaySettings::_instance->m_sendMsgQueue.push(msgInfo);
                DisplaySettings::_instance->m_sendMsgThreadRun = true;
	...
	void DisplaySettings::sendArcTerminationRequestMsg ()
	{
		SendMsgInfo msgInfo;
			
		msgInfo.msg = SEND_REQUEST_ARC_TERMINATION;
		msgInfo.param = -1;
		std::unique_lock<std::mutex> lock(DisplaySettings::_instance->m_sendMsgMutex);
		DisplaySettings::_instance->m_sendMsgQueue.push(msgInfo);
		DisplaySettings::_instance->m_sendMsgThreadRun = true;
	...
	void DisplaySettings::sendArcInitiationRequestMsg ()
	{
		SendMsgInfo msgInfo;
			
		msgInfo.msg = SEND_REQUEST_ARC_INITIATION;
		msgInfo.param = -1;
		std::unique_lock<std::mutex> lock(DisplaySettings::_instance->m_sendMsgMutex);
		DisplaySettings::_instance->m_sendMsgQueue.push(msgInfo);
		DisplaySettings::_instance->m_sendMsgThreadRun = true;
	...
	void DisplaySettings::sendAudioDevicePowerOnMsg()
	{
		SendMsgInfo msgInfo;

		msgInfo.msg = SEND_AUDIO_DEVICE_POWERON_MSG;
		msgInfo.param = -1;
		std::unique_lock<std::mutex> lock(DisplaySettings::_instance->m_sendMsgMutex);
		DisplaySettings::_instance->m_sendMsgQueue.push(msgInfo);
		DisplaySettings::_instance->m_sendMsgThreadRun = true;
...
void DisplaySettings::sendMsgThread()
{
	...
	{
                LOGINFO("%s: Debug: Send message Thread wait \n",__FUNCTION__);
		...
		if( !value.compare("success") ) {
		    ...
		    if ((m_AudioDeviceSADState == AUDIO_DEVICE_SAD_CLEARED || m_AudioDeviceSADState == AUDIO_DEVICE_SAD_UNKNOWN) && \
			   ...
			   LOGINFO("%s: Initiate SAD request\n", __FUNCTION__);
			   m_AudioDeviceSADState = AUDIO_DEVICE_SAD_REQUESTED;
		           m_requestSad = true; 
			   /*
                           std::lock_guard<std::mutex> lock(m_arcRoutingStateMutex);
                           ...
                           arcRoutingCV.notify_one();
			   */
			   sendShortAudioDescriptorRequestMsg();
                                
		    } else {
			    LOGINFO("%s: SAD not requested m_AudioDeviceSADState =%d", __FUNCTION__, m_AudioDeviceSADState);
		    }
		    //update device type in case we receive ARC init before power ON request
		    if (m_hdmiInAudioDeviceType == dsAUDIOARCSUPPORT_NONE) {
			    LOGINFO("%s: Updating Audio device type to Arc\n", __FUNCTION__);
			...
			if(m_hdmiInAudioDeviceConnected ==  false) {
				...
				LOGINFO("Arc Initiation sucess, Notify UI\n");
				    ...
				    LOGINFO("%s: not notified to UI since m_arcAudioEnabled =%d\n", __FUNCTION__, m_arcAudioEnabled);
			    }
			}
			else {
		...
		if (m_AudioDeviceSADState == AUDIO_DEVICE_SAD_REQUESTED) {
			...
			if (m_SADDetectionTimer.isActive()) {
			    ...
			    if (m_arcAudioEnabled == false) {
				LOGINFO("%s: Routing the audio since m_arcAudioEnabled = %d\n", __FUNCTION__, m_arcAudioEnabled);
				 aPort.enableARC(dsAUDIOARCSUPPORT_ARC, true);
                        	 m_arcAudioEnabled = true;
			    }
			} else if (m_arcAudioEnabled == true) {
				...
				LOGINFO("%s: Not updating SAD now since arc routing has not happened yet and SAD timer is not active\n",__FUNCTION__);
			}
                    }
                    catch (const device::Exception& err)
                    {
        ...
        void DisplaySettings::onSystemAudioModeEventHandler(const JsonObject& parameters) {
			...
			*/
			...
			sendAudioDevicePowerStatusRequestMsg();
			...
			std::lock_guard<std::mutex> lock(m_ArcDisableMutex);
			    ...
			    m_hdmiInAudioDeviceConnected = false;
			    if (m_arcAudioEnabled == true) {
				LOGINFO("System Audio mode is off and arc Enable is %d, Notify UI to disbale Arc", m_arcAudioEnabled);
		            	connectedAudioPortUpdated(dsAUDIOPORT_TYPE_HDMI_ARC, false);
			    }
                            {
	...
	void DisplaySettings::onAudioDevicePowerStatusEventHandler(const JsonObject& parameters) {
	     ...
	     LOGINFO("Audio Device Power State [%d] ... \n", pState);
			...
			if ((m_hdmiInAudioDeviceConnected == false) && !(m_ArcDetectionTimer.isActive())) {
             ...
             else if(pState == POWER_STATE_STANDBY) {
                 m_hdmiInAudioDevicePowerState = AUDIO_DEVICE_POWER_STATE_STANDBY;
             } else if (pState == POWER_STATE_IN_TRANSITION) {
		     //Start a timer to re check the power status of AVR?
		     LOGINFO("Audio device  power status IN TRANSITION from STANDBY to ON, Requesting power status again\n");
		     /*
			std::lock_guard<std::mutex> lock(m_arcRoutingStateMutex);
                        m_cecArcRoutingThreadRun = true;
                        arcRoutingCV.notify_one();
	...
	/*Arc detection timer*/
	    ...
	    if((types & dsAUDIOARCSUPPORT_eARC) && (m_hdmiInAudioDeviceConnected == false)) {
		   ...
		   if (m_arcAudioEnabled == false) {
			   ...
			   LOGINFO("Arc enabled already m_arcAudioEnabled =%d", m_arcAudioEnabled);
		   }
			...
			*/
			...
			sendArcInitiationRequestMsg();
			
                    } else {
			    LOGINFO("%s: Arc initiation request not sent\n", __FUNCTION__);
		    }
	    }
